...
bool ImGui::HotKey(const char* label, int* k, ImFont* fontsize, const ImVec2& size_arg)
{
	...
	//ImGui::GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max - ImVec2(5,5), ImGui::GetColorU32(ImGui::GetStyleColorVec4(ImGuiCol_ButtonHovered)));

	if (*k != 0 && g.ActiveId != id)
		buf_display = KeyNames[*k];					//strcpy_s(buf_display, KeyNames[*k]);
	else if (g.ActiveId == id)
		buf_display = "press a key";				//strcpy_s(buf_display, "Press a key");
	else
		buf_display = "none";						//strcpy_s(buf_display, "None");
	buf_display.insert(0, "[");
	buf_display += "]";

	const ImRect clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + size.x, frame_bb.Min.y + size.y);
	ImVec2 render_pos = frame_bb.Min + style.FramePadding;
	buf_display.insert(0, "               "); // sihttyfix

	ImGui::PushFont(fontsize);
	ImGui::PushStyleColor(ImGuiCol_Text, ImGui::GetStyleColorVec4(ImGuiCol_Text));
	ImGui::RenderTextClipped(frame_bb.Min - ImVec2(80, 0), frame_bb.Max - style.FramePadding, buf_display.c_str(), NULL, NULL, style.ButtonTextAlign, &clip_rect);
	ImGui::PopStyleColor();
	ImGui::PopFont();
